...
// ─────────────────────────────────────────────────────────────────────────────
// LIFE ORG — Deployment-ready family calendar
// ─────────────────────────────────────────────────────────────────────────────

const APP_NAME    = "Life Org";
...
// ── Victorian school terms & public holidays ──────────────────────────────────
const SEED_TERMS = [
  ...
  birthdays.forEach(bd=>{
    ...
    [y,y+1].forEach(yr=>{
      ...
      evs.push({id:`bd-${bd.id}-${yr}`,title:getBdTitle(bd,yr),type:"birthday",
        ...
        recurring:"yearly",isBirthday:true,bdCategory:bd.category||"friend"});
    });
  });
  return evs;
}
function bdThisWeek(ds){const diff=Math.round((parseDate(ds)-new Date())/86400000);return diff>=0&&diff<=7;}
...
function generateICal(events,birthdays,terms,name){
  ...
  const allEvs=[...events,...birthdayEvents(birthdays)];
  allEvs.forEach(ev=>{
    ...
    if(ev.allDay||!ev.startTime){
      ...
      lines.push(`DTEND;VALUE=DATE:${fmt(addDays(parseDate(ev.endDate||ev.date),1)).replace(/-/g,"")}`);
    }else{
      lines.push(`DTSTART:${icalDate(ev.date,ev.startTime)}`);
      lines.push(`DTEND:${icalDate(ev.date,ev.endTime||addMins(ev.startTime,60))}`);
    }
  ...
  terms.forEach((term,i)=>{
    ...
    lines.push("BEGIN:VEVENT",`UID:term-${i}@lifeorg`,`SUMMARY:📚 ${term.label}`,
      `DTSTART;VALUE=DATE:${term.start.replace(/-/g,"")}`,
      `DTEND;VALUE=DATE:${fmt(addDays(parseDate(term.end),1)).replace(/-/g,"")}`,
      "END:VEVENT");
  });
  Object.entries(VIC_HOLIDAYS).forEach(([ds,name])=>{
    lines.push("BEGIN:VEVENT",`UID:ph-${ds}@lifeorg`,`SUMMARY:🏖️ ${name}`,
      `DTSTART;VALUE=DATE:${ds.replace(/-/g,"")}`,
      `DTEND;VALUE=DATE:${fmt(addDays(parseDate(ds),1)).replace(/-/g,"")}`,
      "END:VEVENT");
  });
  lines.push("END:VCALENDAR");
  return lines.join("\r\n");
}
function downloadICal(ical,filename){
...
const t=n=>fmt(addDays(new Date(),n));
...
const SEED_EVENTS=[
  ...
  {id:2,title:"Football Training",type:"sport",       members:["dad","myles"],        date:t(1), endDate:t(1), allDay:false,startTime:"17:00",endTime:"18:00",notes:"",recurring:"none"},
  {id:3,title:"Dentist",          type:"appointment", members:["grace"],              date:t(2), endDate:t(2), allDay:false,startTime:"10:00",endTime:"10:30",notes:"Grace's check-up",recurring:"none"},
  {id:4,title:"Team Meeting",     type:"work",        members:["dad"],               date:t(1), endDate:t(1), allDay:false,startTime:"09:00",endTime:"10:00",notes:"",recurring:"none"},
  {id:5,title:"Electricity Bill", type:"bills",       members:["mum"],               date:t(4), endDate:t(4), allDay:true, startTime:"",    endTime:"",    notes:"$180 due",recurring:"none"},
  {id:6,title:"Family Vacation",  type:"holiday",     members:["all"],               date:t(5), endDate:t(10),allDay:true, startTime:"",    endTime:"",    notes:"Gold Coast 🌊",recurring:"none"},
  {id:7,title:"Marley — Groomer", type:"pet",         members:["marley","mum"],      date:t(3), endDate:t(3), allDay:false,startTime:"09:00",endTime:"10:00",notes:"Full groom + nail trim",recurring:"none"},
];
...
const blankForm=(pre)=>({id:null,type:null,title:"",customTitle:false,members:[],
  date:pre||today,endDate:pre||today,allDay:false,startTime:"",endTime:"",
  notes:"",recurring:"none",bdName:"",bdDob:"",bdCategory:"friend",bdMemberId:""});

// ── Style tokens ──────────────────────────────────────────────────────────────
const lbl={fontSize:12,fontWeight:700,color:"#6b7280",letterSpacing:0.5,textTransform:"uppercase",display:"block",marginBottom:8};
const inp={width:"100%",border:"1.5px solid #e5e7eb",borderRadius:10,padding:"10px 12px",fontSize:14,outline:"none",boxSizing:"border-box",background:"#fff"};
...
export default function LifeOrg(){
  ...
  const [marleyInfo,   setMarleyInfo]   = useState({vet:"",groomer:"",nextVac:"",nextFlea:""});

  const [view,         setView]         = useState("splash");  // splash → week
  ...
  // ── Splash → week after 2s ────────────────────────────────────────────────
  useEffect(()=>{
    const timer=setTimeout(()=>setView("week"),2200);
...
useEffect(()=>{
  ...
  const getMember =id=>MEMBERS_ALL.find(m=>m.id===id);
  ...
  const holidayForDay=useCallback(ds=>VIC_HOLIDAYS[ds]||null,[]);
  ...
  function openEdit(ev){
    setForm({id:ev.id,type:ev.type,title:ev.title,customTitle:true,members:ev.members,
      date:ev.date,endDate:ev.endDate||ev.date,allDay:ev.allDay||false,
      startTime:ev.startTime||"",endTime:ev.endTime||"",notes:ev.notes||"",
      recurring:ev.recurring||"none",bdName:"",bdDob:"",bdCategory:"friend",bdMemberId:""});
    setEditingId(ev.id);setStep(3);setView("add");setExpandedEv(null);
  }
  function confirmDelete(id,title){setDeleteConfirm({id,title});}
  function doDelete(){if(!deleteConfirm)return;setEvents(prev=>prev.filter(e=>e.id!==deleteConfirm.id));setExpandedEv(null);showToast("🗑 Event removed");setDeleteConfirm(null);}
  ...
  function submitEvent(){
    if(form.type==="birthday"){
      ...
      const nb={id:`bd${Date.now()}`,name:form.bdName,dob:form.bdDob,notes:form.notes,category:form.bdCategory,memberId:form.bdMemberId||null};
      setBirthdays(prev=>[...prev,nb]);showToast("🎂 Birthday saved!");
      setTimeout(()=>{setView("week");setStep(1);setForm(blankForm());},1600);return;
    }
    ...
    if(editingId){setEvents(prev=>prev.map(e=>e.id===editingId?{...e,...payload,id:editingId}:e));showToast("✅ Event updated!");}
    else{setEvents(prev=>[...prev,{...payload,id:Date.now()}]);showToast("✅ Added to calendar!");}
    setTimeout(()=>{setView("week");setStep(1);setForm(blankForm());setEditingId(null);},1600);
  }
  function addNote(){if(!newNote.trim())return;setNotes(prev=>[{id:Date.now(),text:newNote.trim(),createdAt:today,pinned:false},...prev]);setNewNote("");}
  function saveBirthday(){
    if(!bdForm?.name||!bdForm?.dob)return;
    const ex=birthdays.find(b=>b.id===bdForm.id);
    if(ex)setBirthdays(prev=>prev.map(b=>b.id===bdForm.id?{...b,...bdForm}:b));
    else setBirthdays(prev=>[...prev,{...bdForm,id:`bd${Date.now()}`}]);
  ...
  if(view==="splash"){
    return(
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"linear-gradient(145deg,#4f46e5 0%,#7c3aed 100%)",fontFamily:"'Inter',system-ui,sans-serif"}}>
        {/* Animated logo mark */}
        <div style={{marginBottom:24,animation:"splashPop 0.6s cubic-bezier(0.34,1.56,0.64,1) forwards"}}>
          <svg width="96" height="96" viewBox="0 0 96 96">
            ...
            <circle cx="28" cy="23" r="5" fill="#7c3aed"/>
            <circle cx="28" cy="23" r="3" fill="#e0e7ff"/>
            <circle cx="68" cy="23" r="5" fill="#7c3aed"/>
            <circle cx="68" cy="23" r="3" fill="#e0e7ff"/>
            {[0,1,2,3,4,5,6].map(i=>(
              <text key={i} x={18+i*11} y={56} textAnchor="middle" fontSize="9" fontWeight="600" fill="#9ca3af" fontFamily="system-ui">{["M","T","W","T","F","S","S"][i]}</text>
            ))}
            ...
            {[[26,70],[37,70],[59,70],[70,70]].map(([x,y],i)=>(
              <text key={i} x={x} y={y} textAnchor="middle" fontSize="10" fontWeight="500" fill="#374151" fontFamily="system-ui">{[2,3,6,7][i]}</text>
            ))}
            <circle cx="26" cy="76" r="2.5" fill="#ec4899"/>
            <circle cx="59" cy="76" r="2.5" fill="#10b981"/>
          </svg>
        </div>
        <div style={{color:"#fff",fontSize:34,fontWeight:800,letterSpacing:"-0.5px",marginBottom:6}}>Life Org</div>
        <div style={{color:"rgba(255,255,255,0.65)",fontSize:15,fontWeight:400,letterSpacing:"0.5px"}}>{APP_TAGLINE}</div>

        {/* Loading dots */}
        <div style={{display:"flex",gap:8,marginTop:48}}>
          {[0,1,2].map(i=>(
            <div key={i} style={{width:8,height:8,borderRadius:"50%",background:"rgba(255,255,255,0.4)",animation:`dot 1.2s ${i*0.2}s ease-in-out infinite`}}/>
          ))}
        </div>
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // SHARE / WEEK SUMMARY CARD
  // ─────────────────────────────────────────────────────────────────────────
  if(shareMode){
    const shareEvs=weekDays.flatMap(d=>{
      const ds=fmt(d);
      return(eventsByDay[ds]||[]).map(ev=>({...ev,_ds:ds,_day:d}));
    });
    return(
      <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"#f8f7ff",minHeight:"100vh",maxWidth:480,margin:"0 auto"}}>
        ...
        {/* Member summary row */}
        <div style={{display:"flex",gap:0,background:"#fff",borderBottom:"1px solid #f3f4f6"}}>
          {settings.members.map(m=>(
            <div key={m.id} style={{flex:1,textAlign:"center",padding:"12px 4px"}}>
              <div style={{fontSize:22,marginBottom:2}}>{m.emoji}</div>
              <div style={{fontSize:9,fontWeight:700,color:"#374151"}}>{m.name}</div>
              <div style={{fontSize:20,fontWeight:800,color:m.color}}>{memberCounts[m.id]||0}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{padding:"16px 20px"}}>
          {weekDays.map(day=>{
            ...
            const ph=holidayForDay(ds);
            const isToday=ds===today;
            if(!dayEvs.length&&!ph)return null;
            return(
              <div key={ds} style={{marginBottom:14}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  <div style={{fontSize:13,fontWeight:800,color:isToday?APP_COLOR:"#374151"}}>
                    {day.toLocaleDateString("en-AU",{weekday:"long",day:"numeric",month:"short"})}
                    {isToday&&<span style={{marginLeft:6,fontSize:10,color:APP_COLOR,fontWeight:700}}>TODAY</span>}
                  </div>
                </div>
                {ph&&<div style={{fontSize:11,color:"#dc2626",fontWeight:700,marginBottom:4}}>🏖️ {ph}</div>}
                {dayEvs.map(ev=>{
                  const type=getType(ev.type);
                  const evMs=ev.members.includes("all")?settings.members:ev.members.map(getMember).filter(Boolean);
                  return(
                    <div key={ev.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"#fff",borderRadius:10,marginBottom:4,borderLeft:`3px solid ${type?.color||"#6366f1"}`,boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
                      <span style={{fontSize:16}}>{ev.isBirthday?"🎂":type?.emoji}</span>
                      <div style={{flex:1}}>
                        <div style={{fontSize:13,fontWeight:700,color:"#1f1535"}}>{ev.title}</div>
                        {!ev.allDay&&ev.startTime&&<div style={{fontSize:11,color:"#6b7280"}}>{fmtTime(ev.startTime)}{ev.endTime?` – ${fmtTime(ev.endTime)}`:""}</div>}
                      </div>
          ...
          {shareEvs.length===0&&(
            <div style={{textAlign:"center",padding:"40px 0",color:"#9ca3af"}}>
              ...
              <div style={{fontSize:14,fontWeight:600}}>Nothing on this week!</div>
              <div style={{fontSize:12,marginTop:4}}>Enjoy the free time</div>
            </div>
          )}
        </div>
        ...
        <div style={{padding:"10px 12px",display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
              {ev.allDay?<span style={{fontSize:11,color:"#6b7280",fontWeight:600}}>All day</span>
                :ev.startTime&&<span style={{fontSize:11,color:"#6b7280",fontWeight:600}}>🕐 {fmtTime(ev.startTime)}{ev.endTime?` – ${fmtTime(ev.endTime)}`:""}</span>}
  ...
  // ─────────────────────────────────────────────────────────────────────────
  function WeekView(){
    const totalWeekEvents=weekDays.reduce((s,d)=>{const ds=fmt(d);return s+(eventsByDay[ds]||[]).length+(filterMember==="all"?(gardenByDay[ds]||[]).length:0);},[].length+0);
    return(
      <div style={{padding:"0 0 20px"}}>
        ...
        {upcomingBds.length>0&&(
          <div style={{margin:"0 20px 12px",background:"linear-gradient(135deg,#fef3c7,#fde68a)",borderRadius:12,padding:"10px 14px",border:"1px solid #fcd34d"}}>
            <div style={{fontSize:12,fontWeight:800,color:"#92400e"}}>🎂 {upcomingBds.map(b=>b.title).join("  ·  ")}</div>
          </div>
        )}

        {/* Empty state — full week */}
        {totalWeekEvents===0&&(
          ...
          return(
            <div key={ds} style={{margin:"0 20px 8px"}}>
              ...
              <div style={termBg&&!isEmpty?{background:termBg,borderRadius:10,padding:"4px 6px"}:{}}>
                {dayEvs.map(ev=><EventPill key={ev.id+ds} ev={ev} dateStr={ds}/>)}
      ...
      <div style={{padding:"0 20px 30px"}}>
        ...
        {step===3&&(
          <>
            ...
            {form.type==="birthday"&&(
              <>
                <div style={{marginBottom:16}}><label style={lbl}>Whose birthday?</label><input value={form.bdName} onChange={e=>setForm(f=>({...f,bdName:e.target.value}))} placeholder="e.g. Uncle Luke, Grandma Sue..." style={inp}/></div>
                <div style={{marginBottom:16}}>
                  <label style={lbl}>Date of birth</label>
                  <input type="date" value={form.bdDob} onChange={e=>setForm(f=>({...f,bdDob:e.target.value}))} style={inp}/>
                  {form.bdName&&form.bdDob&&(
                    <div style={{marginTop:8,background:"#fdf2f8",borderRadius:10,padding:"10px 14px",border:"1px solid #fbcfe8"}}>
                      <div style={{fontSize:12,color:"#9d174d",fontWeight:700,marginBottom:2}}>Preview:</div>
                      <div style={{fontSize:14,fontWeight:700,color:"#1f1535"}}>{getBdTitle({name:form.bdName,dob:form.bdDob},new Date().getFullYear())}</div>
                      <div style={{fontSize:11,color:"#6b7280",marginTop:3}}>🔁 Repeats every year · Age updates automatically</div>
                    </div>
                  )}
                </div>
                <div style={{marginBottom:16}}>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {[{id:"family",label:"Family",emoji:"🏠"},{id:"friend",label:"Friend",emoji:"👥"},{id:"work",label:"Work",emoji:"💼"},{id:"other",label:"Other",emoji:"📌"}].map(c=>(
                      <button key={c.id} onClick={()=>setForm(f=>({...f,bdCategory:c.id}))} style={{background:form.bdCategory===c.id?"#f43f5e":"#fff",color:form.bdCategory===c.id?"#fff":"#374151",border:`1.5px solid ${form.bdCategory===c.id?"#f43f5e":"#e5e7eb"}`,borderRadius:20,padding:"6px 14px",cursor:"pointer",fontSize:13,fontWeight:600}}>{c.emoji} {c.label}</button>
                    ))}
                  </div>
                </div>
                <div style={{marginBottom:16}}>
                  <label style={lbl}>Link to family member <span style={{fontWeight:400,fontSize:11,textTransform:"none"}}>(optional)</span></label>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                ...
                <div style={{marginBottom:24}}><label style={lbl}>Notes (optional)</label><input value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="e.g. Surprise party planned!" style={inp}/></div>
            ...
            {form.type!=="birthday"&&(
              <>
                {(form.customTitle||isEdit)?(<div style={{marginBottom:16}}><label style={lbl}>Title</label><input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="Event name" style={inp}/></div>)
                ...
                <div style={{marginBottom:16}}>
                  <label style={lbl}>Who's involved?</label>
                ...
                <div style={{marginBottom:16}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    <div><div style={{fontSize:11,color:"#9ca3af",marginBottom:4,fontWeight:600}}>Start</div><input type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value,endDate:e.target.value>f.endDate?e.target.value:f.endDate}))} style={inp}/></div>
                    <div><div style={{fontSize:11,color:"#9ca3af",marginBottom:4,fontWeight:600}}>End</div><input type="date" value={form.endDate} min={form.date} onChange={e=>setForm(f=>({...f,endDate:e.target.value}))} style={inp}/></div>
                  </div>
                ...
                {!form.allDay&&(
                  <div style={{marginBottom:16}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                      <div><div style={{fontSize:11,color:"#9ca3af",marginBottom:4,fontWeight:600}}>Start</div><input type="time" value={form.startTime} onChange={e=>setForm(f=>({...f,startTime:e.target.value}))} style={inp}/></div>
                      <div><div style={{fontSize:11,color:"#9ca3af",marginBottom:4,fontWeight:600}}>End <span style={{fontSize:9,opacity:0.6}}>(1hr default)</span></div><input type="time" value={form.endTime} onChange={e=>setForm(f=>({...f,endTime:e.target.value}))} style={inp}/></div>
                    </div>
                  </div>
                )}
                ...
                <div style={{marginBottom:24}}><label style={lbl}>Notes (optional)</label><textarea value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="Any extra details..." rows={2} style={{...inp,resize:"none",fontFamily:"inherit"}}/></div>
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // NOTES / MARLEY / BIRTHDAYS / SETTINGS views (compact)
  // ─────────────────────────────────────────────────────────────────────────
  function NotesView(){
      ...
      <div style={{padding:"12px 20px 20px"}}>
        ...
        {sorted.length===0&&(
          <div style={{textAlign:"center",padding:"40px 0",color:"#9ca3af"}}>
            ...
            <div style={{fontSize:12,marginTop:4}}>Add a quick reminder above</div>
          </div>
        )}
        {sorted.map(n=>(
          <div key={n.id} style={{background:n.pinned?"#faf5ff":"#fff",borderRadius:12,padding:"12px 14px",marginBottom:8,border:n.pinned?"1.5px solid #c4b5fd":"1.5px solid #f3f4f6",boxShadow:"0 1px 3px rgba(0,0,0,0.05)",display:"flex",alignItems:"flex-start",gap:10}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:4,flexShrink:0}}>
              <button onClick={()=>setNotes(prev=>prev.map(nn=>nn.id===n.id?{...nn,pinned:!nn.pinned}:nn))} style={{background:"none",border:"none",fontSize:14,cursor:"pointer",color:n.pinned?"#7c3aed":"#d1d5db"}}>📌</button>
              <button onClick={()=>setNotes(prev=>prev.filter(nn=>nn.id!==n.id))} style={{background:"none",border:"none",fontSize:14,cursor:"pointer",color:"#d1d5db"}}>✕</button>
            </div>
          </div>
        ))}
      </div>
    ...
    return(
      <div style={{padding:"12px 20px 20px"}}>
        ...
        <div style={{background:"#fff",borderRadius:14,padding:"14px 16px",marginBottom:16,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          ...
          {[{key:"vet",label:"Vet clinic",ph:"e.g. Paws Vet Clinic"},{key:"groomer",label:"Groomer",ph:"e.g. Doggy Styles"},{key:"nextVac",label:"Next vaccination",ph:"YYYY-MM-DD"},{key:"nextFlea",label:"Next flea treatment",ph:"YYYY-MM-DD"}].map(f=>(
            <div key={f.key} style={{marginBottom:10}}>
              ...
              <input value={marleyInfo[f.key]} onChange={e=>setMarleyInfo(p=>({...p,[f.key]:e.target.value}))} placeholder={f.ph} style={{width:"100%",border:"1.5px solid #e5e7eb",borderRadius:8,padding:"8px 12px",fontSize:13,outline:"none",boxSizing:"border-box"}}/>
            </div>
          ))}
        </div>
        <div style={{background:"#fff",borderRadius:14,padding:"14px 16px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
          ...
          {petEvs.length===0
            ?<div style={{textAlign:"center",color:"#9ca3af",fontSize:13,padding:"20px 0"}}><div style={{fontSize:32,marginBottom:8}}>🐾</div>No upcoming events for {pet.name}</div>
            :petEvs.map(ev=>{const away=Math.round((parseDate(ev.date)-new Date())/86400000);return(
              <div key={ev.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:"1px solid #f9fafb"}}>
                <span style={{fontSize:18}}>{getType(ev.type)?.emoji||"📌"}</span>
          ...
          <button onClick={()=>{setForm({...blankForm(),type:"pet",members:pet.id?[pet.id]:[]});setView("add");setStep(2);}} style={{marginTop:12,width:"100%",background:"#fef3c7",color:"#92400e",border:"none",borderRadius:10,padding:"10px",fontSize:13,fontWeight:700,cursor:"pointer"}}>+ Book {pet.name} an appointment</button>
        </div>
      </div>
    );
  }
  ...
  function BirthdaysView(){
    return(
      <div style={{padding:"12px 20px 20px"}}>
        ...
        {upcomingBds.length>0&&(
          <div style={{background:"linear-gradient(135deg,#fef3c7,#fde68a)",borderRadius:12,padding:"12px 14px",marginBottom:14,border:"1px solid #fcd34d"}}>
            <div style={{fontSize:11,fontWeight:800,color:"#92400e",marginBottom:6}}>🎉 COMING UP THIS WEEK</div>
            {upcomingBds.map(b=><div key={b.id} style={{fontSize:13,color:"#78350f",fontWeight:600,marginBottom:2}}>{b.title}</div>)}
          </div>
        )}
        {birthdays.length===0&&(
          <div style={{textAlign:"center",padding:"40px 0",color:"#9ca3af"}}>
            <div style={{fontSize:40,marginBottom:10}}>🎂</div>
            <div style={{fontSize:14,fontWeight:600}}>No birthdays added yet</div>
            <div style={{fontSize:12,marginTop:4}}>Tap + Add to get started</div>
          </div>
        )}
          ...
          return(
            <div key={cat} style={{marginBottom:16}}>
              ...
              {bds.slice().sort((a,b)=>(a.dob?a.dob.slice(5):"99").localeCompare(b.dob?b.dob.slice(5):"99")).map(bd=>{
                const m=bd.memberId?getMember(bd.memberId):null;
                  ...
                  <div key={bd.id} style={{background:"#fff",borderRadius:12,padding:"11px 14px",marginBottom:8,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",borderLeft:away!==null&&away<=7?"3px solid #f43f5e":"3px solid #f3f4f6",display:"flex",alignItems:"center",gap:10}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                      ...
                      <div style={{display:"flex",gap:4}}>
                        ...
                        <button onClick={()=>setBirthdays(prev=>prev.filter(b=>b.id!==bd.id))} style={{background:"none",border:"none",color:"#fca5a5",cursor:"pointer",fontSize:14}}>✕</button>
                      </div>
                    </div>
                  </div>
                );
  ...
  function SettingsView(){
    ...
    return(
      <div style={{padding:"12px 20px 30px"}}>
        ...
        <Sec title="App name">
          <input value={ls.familyName} onChange={e=>setLs(s=>({...s,familyName:e.target.value}))} placeholder="e.g. The Smiths" style={inp}/>
        ...
        <Sec title="Family members">
          {ls.members.map(m=>(
            <div key={m.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid #f3f4f6"}}>
              ...
              <input value={m.name} onChange={e=>setLs(s=>({...s,members:s.members.map(mm=>mm.id===m.id?{...mm,name:e.target.value}:mm)}))} style={{...inp,width:90,padding:"6px 10px",fontSize:13}}/>
          ...
          <div style={{marginTop:12,padding:"12px",background:"#f9fafb",borderRadius:10}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <input value={nm.name} onChange={e=>setNm(n=>({...n,name:e.target.value}))} placeholder="Name" style={{...inp,flex:1,padding:"8px 10px",fontSize:13}}/>
          ...
          {lt.map((term,i)=>(
            <div key={i} style={{marginBottom:12,background:"#f9fafb",borderRadius:10,padding:"10px 12px",borderLeft:`3px solid ${["#3b82f6","#10b981","#f59e0b","#ec4899"][i%4]}`}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <div><div style={{fontSize:10,color:"#9ca3af",marginBottom:3,fontWeight:600}}>Start</div><input type="date" value={term.start} onChange={e=>setLt(ts=>ts.map((tt,j)=>j===i?{...tt,start:e.target.value}:tt))} style={{...inp,fontSize:12,padding:"7px 10px"}}/></div>
                <div><div style={{fontSize:10,color:"#9ca3af",marginBottom:3,fontWeight:600}}>End</div><input type="date" value={term.end} onChange={e=>setLt(ts=>ts.map((tt,j)=>j===i?{...tt,end:e.target.value}:tt))} style={{...inp,fontSize:12,padding:"7px 10px"}}/></div>
              </div>
            </div>
          ))}
        ...
        <Sec title="📅 Export Calendar">
          ...
          <button onClick={handleExport} style={{width:"100%",background:"linear-gradient(135deg,#4f46e5,#7c3aed)",color:"#fff",border:"none",borderRadius:12,padding:"14px",fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📥 Download .ics Calendar File</button>
        ...
        <button onClick={saveAll} style={{width:"100%",background:saved?"#10b981":"linear-gradient(135deg,#4f46e5,#7c3aed)",color:"#fff",border:"none",borderRadius:14,padding:"16px",fontSize:16,fontWeight:700,cursor:"pointer",marginTop:8,transition:"background 0.3s"}}>{saved?"✅ Saved!":"Save Settings"}</button>
      </div>
    );
  }
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:70,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 30px"}}>
        <div style={{background:"#fff",borderRadius:20,padding:"28px 24px",width:"100%",maxWidth:340,textAlign:"center",boxShadow:"0 20px 60px rgba(0,0,0,0.25)"}}>
          ...
          <div style={{fontSize:14,color:"#6b7280",marginBottom:24}}>
            "<span style={{fontWeight:600,color:"#374151"}}>{deleteConfirm?.title}</span>" will be permanently removed
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>setDeleteConfirm(null)} style={{flex:1,background:"#f3f4f6",border:"none",borderRadius:12,padding:"13px",fontSize:14,fontWeight:700,cursor:"pointer",color:"#374151"}}>Cancel</button>
            <button onClick={doDelete} style={{flex:1,background:"linear-gradient(135deg,#ef4444,#dc2626)",border:"none",borderRadius:12,padding:"13px",fontSize:14,fontWeight:700,cursor:"pointer",color:"#fff"}}>Delete</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:60,display:"flex",alignItems:"flex-end"}}>
        <div style={{background:"#fff",width:"100%",maxWidth:480,margin:"0 auto",borderRadius:"20px 20px 0 0",padding:"24px 20px 40px",maxHeight:"90vh",overflowY:"auto"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
            ...
            <button onClick={()=>{setShowBdForm(false);setBdForm(null);}} style={{background:"#f3f4f6",border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer"}}>✕</button>
          </div>
          <div style={{marginBottom:14}}><label style={lbl}>Name</label><input value={bdForm?.name||""} onChange={e=>setBdForm(f=>({...f,name:e.target.value}))} placeholder="e.g. Uncle Luke" style={inp}/></div>
          ...
          <div style={{marginBottom:14}}>
            <label style={lbl}>Family member <span style={{fontWeight:400,fontSize:11,textTransform:"none"}}>(optional)</span></label>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          ...
          <div style={{marginBottom:24}}><label style={lbl}>Notes (optional)</label><input value={bdForm?.notes||""} onChange={e=>setBdForm(f=>({...f,notes:e.target.value}))} placeholder="e.g. Surprise party planned!" style={inp}/></div>
          <button onClick={saveBirthday} disabled={!bdForm?.name||!bdForm?.dob} style={{width:"100%",background:(!bdForm?.name||!bdForm?.dob)?"#e5e7eb":"linear-gradient(135deg,#f43f5e,#e11d48)",color:(!bdForm?.name||!bdForm?.dob)?"#9ca3af":"#fff",border:"none",borderRadius:14,padding:"16px",fontSize:16,fontWeight:700,cursor:"pointer"}}>Save Birthday</button>
        </div>
      </div>
    );
  }
      ...
      <div style={{background:"linear-gradient(135deg,#4f46e5 0%,#7c3aed 100%)",color:"#fff",padding:"20px 20px 14px",position:"sticky",top:0,zIndex:10}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:22,fontWeight:800,marginTop:2,letterSpacing:"-0.3px"}}>
              <span style={{opacity:0.7,fontWeight:400}}>✦ </span>Life Org
            </div>
          </div>
          {view!=="add"&&(
            <button onClick={()=>openAdd()} style={{background:"rgba(255,255,255,0.2)",border:"1.5px solid rgba(255,255,255,0.4)",color:"#fff",borderRadius:12,padding:"8px 16px",fontSize:14,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
        ...
        {view==="week"&&(
          <div style={{display:"flex",gap:7,marginTop:14,overflowX:"auto",paddingBottom:2}}>
            {MEMBERS_ALL.map(m=>(
              <button key={m.id} onClick={()=>setFilterMember(m.id)} style={{background:filterMember===m.id?"#fff":"rgba(255,255,255,0.15)",color:filterMember===m.id?m.color:"#fff",border:"none",borderRadius:20,padding:"5px 11px",fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>{m.emoji} {m.name}</button>
            ))}
          </div>
        )}
      </div>
